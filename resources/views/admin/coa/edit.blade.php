<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-[#242424]">Revise Financial Classification</h2>
                <p class="text-xs font-semibold text-gray-400 mt-1 uppercase tracking-tight">{{ $account->code }} • Chart of Accounts Revision</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="card-premium p-8 bg-white">
                <form method="POST" action="{{ route('admin.coa.update', $chartOfAccount) }}" class="space-y-8">
                    @csrf
                    @method('PUT')

                    <div class="flex items-center space-x-2 mb-8 pb-4 border-b border-[#F0F0F0]">
                        <div class="w-1.5 h-6 bg-[#0F6CBD] rounded-full"></div>
                        <h3 class="text-xs font-bold text-[#242424] uppercase tracking-widest">Classification Revision Details</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <input id="code" name="code" type="text" value="{{ old('code', $chartOfAccount->code) }}" class="fluent-input w-full font-mono uppercase bg-[#FAFAFA] border-dashed" readonly>
                            <p class="mt-1 text-[9px] text-gray-400 font-bold italic uppercase tracking-tighter">Read-Only Immutable Protocol</p>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="fluent-input fluent-select w-full font-bold">
                                <option value="expense" {{ old('type', $account->type) == 'expense' ? 'selected' : '' }}>Operational Expenditure (Expense)</option>
                                <option value="income" {{ old('type', $account->type) == 'income' ? 'selected' : '' }}>Revenue Stream (Income)</option>
                            </select>
                            ...
                            <select id="director_id" name="director_id" class="fluent-input fluent-select w-full font-bold">
                                ...
                                @foreach($directors as $director)
                                    <option value="{{ $director->id }}"
                                        {{ old('director_id', $account->director_id) == $director->id ? 'selected' : '' }}>
                                        {{ $director->name }} {{ $director->department ? '('.$director->department.')' : '' }}
                                    </option>
                                @endforeach
                            </select>
                            <p class="mt-1 text-[9px] text-gray-400 italic">Leave blank to keep as a shared global category.</p>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name', $account->name) }}" class="fluent-input w-full" placeholder="e.g., Strategic Partnership Travel" required>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="description" name="description" class="fluent-input w-full h-24" placeholder="Brief context for this account head...">{{ old('description', $account->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input id="budget_limit" name="budget_limit" type="number" step="1" value="{{ old('budget_limit', $account->budget_limit) }}" class="fluent-input w-full pl-12 font-bold" placeholder="0">
                            </div>
