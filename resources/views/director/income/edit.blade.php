<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-[#242424]">Edit Income Report</h2>
                <p class="text-xs font-semibold text-gray-400 mt-1 uppercase tracking-tight">Manual Revenue Revision v4.2</p>
            </div>
            <div class="flex items-center space-x-3">
                <span class="px-2 py-0.5 bg-amber-50 border border-amber-200 rounded-[4px] text-[10px] font-bold text-amber-600 uppercase">Status: {{ $incomeReport->status }}</span>
            </div>
        </div>
    </x-slot>

    <div class="space-y-6" x-data="incomeForm()">
        <form action="{{ route('director.income.update', $incomeReport) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="card-premium p-8 bg-white border border-[#E0E0E0]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                    <div class="md:col-span-8">
                        ...
                        <x-text-input id="title" name="title" type="text" :value="old('title', $incomeReport->title)" required />
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
                        ...
                        <div class="flex items-center space-x-2">
                            <select name="period_month" class="fluent-input fluent-select flex-grow text-[11px] font-bold">
                                @foreach(range(1, 12) as $m)
                                    <option value="{{ $m }}" {{ old('period_month', $incomeReport->period_month) == $m ? 'selected' : '' }}>{{ date("F", mktime(0, 0, 0, $m, 1)) }}</option>
                                @endforeach
                            </select>
                            <select name="period_year" class="fluent-input fluent-select w-24 text-[11px] font-bold">
                                @foreach(range(date('Y')-1, date('Y')+1) as $y)
                                    <option value="{{ $y }}" {{ old('period_year', $incomeReport->period_year) == $y ? 'selected' : '' }}>{{ $y }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div>
                        ...
                        <p class="text-xs font-bold text-[#242424] px-1">{{ $incomeReport->director->name }}</p>
                    </div>
                    <div>
                        <x-input-label for="notes" value="Notes" class="text-[10px] uppercase tracking-widest text-gray-500 font-bold mb-2" />
                        <textarea id="notes" name="notes" rows="1" class="fluent-input text-[11px] h-[33px] resize-none" placeholder="Optional notes...">{{ old('notes', $incomeReport->notes) }}</textarea>
                    </div>
                </div>
            </div>
                                    ...
                                    <td class="px-8 py-3">
                                        <select :name="'lines['+index+'][category_id]'" x-model="line.category_id" class="fluent-input fluent-select text-[11px] font-bold bg-transparent border-transparent hover:border-[#E0E0E0] focus:bg-white w-full" required>
                                            ...
                                            <template x-for="cat in availableCategories" :key="cat.id">
                                                <option :value="cat.id" :selected="cat.id == line.category_id" x-text="cat.name"></option>
                                            </template>
                                        </select>
                                    </td>
            ...
            <div class="flex flex-col md:flex-row items-center justify-end gap-3 p-6 card-premium bg-white border border-[#E0E0E0]">
                ...
                <button type="submit" 
                        @click.prevent="$dispatch('open-confirm', { 
                            title: 'Revise Revenue', 
                            message: 'Submit this revised report for approval?', 
    ...
    <script>
        function incomeForm() {
            return {
                selectedDirectorId: '{{ $incomeReport->director_id }}',
                availableCategories: @json($categories),
                lines: {!! json_encode(old('lines', $incomeReport->lines->map(fn($l) => ['date' => $l->date->format('Y-m-d'), 'description' => $l->description, 'category_id' => $l->category_id, 'amount' => $l->amount]))) !!},
                grandTotal: 0,
                init() {
